...
def generate_pdf(request):
    if request.method == 'POST':
        ...
        for region in regions:
            ...
            # Calculate attributes for each region
            somme_stock_ini = fichiers_par_region.aggregate(somme_stock_ini=Coalesce(Sum('stock_ini'), Value(0), output_field=FloatField()))['somme_stock_ini']
            somme_apport_consommation = fichiers_par_region.aggregate(somme_apport_consommation=Coalesce(Sum('Apport_consommation'), Value(0), output_field=FloatField()))['somme_apport_consommation']
            somme_produit = fichiers_par_region.aggregate(somme_produit=Coalesce(Sum('produit'), Value(0), output_field=FloatField()))['somme_produit']
            somme_consomme = fichiers_par_region.aggregate(somme_consomme=Coalesce(Sum('consomme'), Value(0), output_field=FloatField()))['somme_consomme']
            somme_preleve = fichiers_par_region.aggregate(somme_preleve=Coalesce(Sum('preleve'), Value(0), output_field=FloatField()))['somme_preleve']
            somme_pertes = fichiers_par_region.aggregate(somme_pertes=Coalesce(Sum('pertes'), Value(0), output_field=FloatField()))['somme_pertes']
            somme_expedie = fichiers_par_region.aggregate(somme_expedie=Coalesce(Sum('expedie'), Value(0), output_field=FloatField()))['somme_expedie']
            somme_laivraison = fichiers_par_region.aggregate(somme_laivraison=Coalesce(Sum('laivraison'), Value(0), output_field=FloatField()))['somme_laivraison']
            somme_exp_trc = fichiers_par_region.aggregate(somme_exp_trc=Coalesce(Sum(F('expedie') / F('densite')), Value(0), output_field=FloatField()))['somme_exp_trc']
            somme_stock_final = somme_produit - somme_preleve - somme_pertes - somme_expedie + somme_laivraison + somme_stock_ini - somme_apport_consommation
        ...
        table_data = [[region.nom, attributs['somme_stock_ini'], attributs['somme_apport_consommation'], attributs['somme_produit'],
                       attributs['somme_consomme'], attributs['somme_preleve'], attributs['somme_pertes'], attributs['somme_expedie'],
                       attributs['somme_laivraison'], attributs['somme_exp_trc'], attributs['somme_stock_final']] for region, attributs in somme_attributs_par_region.items()]
...
def generate_excel(request):
    if request.method == 'POST':
        ...
        for row_num, region in enumerate(regions, 2):  # Commencer à partir de la ligne 3
            ...
            attributs = [
                region.nom,
                fichiers_par_region.aggregate(somme_stock_ini=Coalesce(Sum('stock_ini'), Value(0), output_field=FloatField()))['somme_stock_ini'],
                fichiers_par_region.aggregate(somme_apport_consommation=Coalesce(Sum('Apport_consommation'), Value(0), output_field=FloatField()))['somme_apport_consommation'],
                fichiers_par_region.aggregate(somme_produit=Coalesce(Sum('produit'), Value(0), output_field=FloatField()))['somme_produit'],
                fichiers_par_region.aggregate(somme_consomme=Coalesce(Sum('consomme'), Value(0), output_field=FloatField()))['somme_consomme'],
                fichiers_par_region.aggregate(somme_preleve=Coalesce(Sum('preleve'), Value(0), output_field=FloatField()))['somme_preleve'],
                fichiers_par_region.aggregate(somme_pertes=Coalesce(Sum('pertes'), Value(0), output_field=FloatField()))['somme_pertes'],
                fichiers_par_region.aggregate(somme_expedie=Coalesce(Sum('expedie'), Value(0), output_field=FloatField()))['somme_expedie'],
                fichiers_par_region.aggregate(somme_laivraison=Coalesce(Sum('laivraison'), Value(0), output_field=FloatField()))['somme_laivraison'],
                fichiers_par_region.aggregate(somme_exp_trc=Coalesce(Sum(F('expedie') / F('densite')), Value(0), output_field=FloatField()))['somme_exp_trc'],
                fichiers_par_region.aggregate(somme_stock_final=Coalesce(Sum(F('produit') - F('preleve') - F('pertes') - F('expedie') + F('laivraison') + F('stock_ini') - F('Apport_consommation')), Value(0), output_field=FloatField()))['somme_stock_final']
            ]
...
def generate_excel_annuele(request):
    if request.method == 'POST':
        ...
        for region in regions:
            ...
            # Calculer somme_stock_final
            somme_stock_final = somme_produit - somme_preleve - somme_pertes - somme_expedie + somme_laivraison + somme_stock_ini
        ...
        headers = ['Année', 'Région', 'Stock initial', 'Apport de consommation interne', 'Production', 'Consommation',
                   'Prélevé', 'Pertes', 'Expédition vers TRC', 'Livraison', 'Expédition vers TRC en m3', 'Stock final']
        for col_num, header in enumerate(headers):
            cell = ws.cell(row=1, column=col_num + 1, value=header)
            cell.font = bold_font
            cell.alignment = center_alignment
            cell.fill = orange_fill

        # Write year value in the second row
        ws.cell(row=2, column=1, value=annee)

        # Write region data
        row_num = 2
        for region, attributs in somme_attributs_par_region.items():
            ws.cell(row=row_num, column=2, value=region.nom)
            ws.cell(row=row_num, column=3, value=attributs['somme_stock_ini'])
            ws.cell(row=row_num, column=4, value=attributs['somme_apport_consommation'])
            ws.cell(row=row_num, column=5, value=attributs['somme_produit'])
            ws.cell(row=row_num, column=6, value=attributs['somme_consomme'])
            ws.cell(row=row_num, column=7, value=attributs['somme_preleve'])
            ws.cell(row=row_num, column=8, value=attributs['somme_pertes'])
            ws.cell(row=row_num, column=9, value=attributs['somme_expedie'])
            ws.cell(row=row_num, column=10, value=attributs['somme_laivraison'])
            ws.cell(row=row_num, column=11, value=attributs['somme_exp_trc'])
            ws.cell(row=row_num, column=12, value=attributs['somme_stock_final'])
            row_num += 1
